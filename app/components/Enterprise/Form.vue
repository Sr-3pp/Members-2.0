...
const handleUpdate = async () => {
  try {
    ...
    await $fetch(`/api/enterprises/${props.initial?._id}`, {
      method: "PATCH",
      body: payload,
    });
  } finally {
  ...
  <UCard>
    <template #header>
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-xl font-semibold">New Enterprise</h1>
          <p class="text-sm text-gray-500">Create a new enterprise record</p>
        </div>
      </div>
    </template>
